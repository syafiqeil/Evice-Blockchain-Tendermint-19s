...
use crate::state::{
    owner_key, balance_key, uri_key, approval_key, 
    operator_approval_key, Event, TokenId,
};
...
pub fn mint(to: Address, token_id: TokenId, token_uri: String) {
    ...
    if storage::read::<Address>(&owner_key(token_id)).is_some() { panic!("Token ID already exists"); }
    ...
    emit_event(Event::Transfer { from: Address::default(), to, token_id });
}
...
pub fn transfer_from(from: Address, to: Address, token_id: TokenId) {
    ...
    if from != owner { panic!("'from' address is not the owner"); }
    ...
    storage::write(&owner_key(token_id), &to);
    storage::write(&approval_key(token_id), &Address::default()); // Hapus approval setelah transfer
...
pub fn approve(to: Address, token_id: TokenId) {
    let caller = env::caller();
    let owner: Address = storage::read(&owner_key(token_id)).expect("Token does not exist");
    if caller != owner { panic!("Only the owner can approve a transfer"); }
    
    storage::write(&approval_key(token_id), &to);
    emit_event(Event::Approval { owner, approved: to, token_id });
}
...
pub fn set_approval_for_all(operator: Address, approved: bool) {
    ...
    emit_event(Event::ApprovalForAll { owner, operator, approved });
}
...
pub fn is_approved_for_all(owner: Address, operator: Address) {
    let is_approved: bool = storage::read(&operator_approval_key(&owner, &operator)).unwrap_or(false);
