...
pub enum CallAction {
    ...
    Mint { to: Address, token_id: TokenId, token_uri: String },
    TransferFrom { from: Address, to: Address, token_id: TokenId },
    Approve { to: Address, token_id: TokenId },
    ...
    GetApproved { token_id: TokenId },
    IsApprovedForAll { owner: Address, operator: Address },
}

// Definisi Event (untuk dicatat di log) 
#[derive(Serialize)]
pub enum Event {
    Transfer { from: Address, to: Address, token_id: TokenId },
    Approval { owner: Address, approved: Address, token_id: TokenId },
