...
#![no_std]
extern crate alloc; // Diperlukan untuk Vec, HashMap, dll tanpa std library
...
pub extern "C" fn main(call_data_ptr: *const u8, call_data_len: u32) {
    ...
    if let Ok(action) = borsh::BorshDeserialize::try_from_slice(&call_data) {
        match action {
            ...
            state::CallAction::TransferFrom { from, to, amount } => handlers::transfer_from(from, to, amount),
