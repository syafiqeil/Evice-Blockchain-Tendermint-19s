...
// Mendeklarasikan tanda tangan fungsi dari `wasm_runtime.rs`
// "API" yang disediakan untuk smart contract
...
pub mod host {
    ...
    // Membaca data mentah dari penyimpanan kontrak
    pub fn get_storage(key: &[u8]) -> Option<Vec<u8>> {
        let mut output = vec![0u8; 1024]; // Buffer sementara 1KB
    ...
    // Menulis data mentah ke penyimpanan kontrak
    pub fn set_storage(key: &[u8], value: &[u8]) {
        unsafe { write_storage(key.as_ptr(), key.len() as u32, value.as_ptr(), value.len() as u32); }
    }

    // Mendapatkan alamat (address) dari akun yang memanggil kontrak ini
    ...
    // FUNGSI SDK BARU
    /// Mengembalikan timestamp dari blok L1 saat ini (dalam milidetik).
    pub fn block_timestamp() -> u64 {
        unsafe { get_block_timestamp() }
    }

    /// Memerintahkan host L1 untuk mentransfer token native dari saldo kontrak ini
    /// ke alamat penerima yang ditentukan.
