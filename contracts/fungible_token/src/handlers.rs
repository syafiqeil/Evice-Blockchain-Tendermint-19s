...
use evice_contract_sdk::{storage, env, Address};
use crate::state::{
    balance_key, allowance_key, Event,
};
...
pub fn transfer(to: Address, amount: u128) {
    let from = env::caller();
    if from == to { env::revert("Cannot transfer to self"); }

    let mut from_balance: u128 = storage::read(&balance_key(&from)).unwrap_or(0);
    if from_balance < amount { env::revert("Insufficient balance"); }
    ...
    emit_event(Event::Transfer { from, to, value: amount });
}

pub fn approve(spender: Address, amount: u128) {
    let owner = env::caller();
    storage::write(&allowance_key(&owner, &spender), &amount);
    emit_event(Event::Approval { owner, spender, value: amount });
}
...
pub fn transfer_from(from: Address, to: Address, amount: u128) {
    ...
    if allowance < amount { env::revert("Insufficient allowance"); }
