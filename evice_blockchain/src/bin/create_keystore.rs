...
struct Cli {
    #[clap(long, help = "Impor kunci publik mentah (hex) yang sudah ada.", requires = "import_private_key")]
    import_public_key: Option<String>,
    #[clap(long, help = "Impor kunci privat mentah (hex) yang sudah ada.", requires = "import_public_key")]
    import_private_key: Option<String>,
    // PERBAIKAN: Tambahkan argumen untuk menerima password non-interaktif
    #[clap(long, help = "Berikan kata sandi secara langsung (hanya untuk skrip pengujian).")]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Kunci Publik (penuh): 0x{}...", &hex::encode(&pub_key_bytes_full[..32]));

    // Gunakan password dari argumen jika ada, jika tidak, minta secara interaktif.
    ...
    let keystore = Keystore::new(&priv_key_bytes_vec.as_slice().try_into()?, &password, &pub_key_bytes_full)?;
    ...
    let filename = format!("UTC--{}--{}", chrono::Utc::now().to_rfc3339(), address_hex_for_filename);
    let path = dir.join(filename);

    keystore.save_to_path(&path)?;
    priv_key_bytes_vec.zeroize();

    println!("\n✅ Keystore berhasil dibuat dan disimpan di: {}", path.display());
    println!("   Alamat (Identifier untuk Nama File): 0x{}", address_hex_for_filename);
    Ok(())
}