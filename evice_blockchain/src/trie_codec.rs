...
// Konstanta yang mendefinisikan bagaimana node di-encode
mod trie_constants {
...
// Helper structs dan fungsi yang dibutuhkan oleh codec
...
pub(crate) enum NodeKind {
	Leaf, BranchNoValue, BranchWithValue, HashedValueLeaf, HashedValueBranch,
}

struct ByteSliceInput<'a> { data: &'a [u8], offset: usize, }
impl<'a> ByteSliceInput<'a> {
	...
	fn take(&mut self, count: usize) -> Result<Range<usize>, codec::Error> {
		if self.offset + count > self.data.len() { return Err("out of data".into()) }
...
impl<'a> Input for ByteSliceInput<'a> {
	fn remaining_len(&mut self) -> Result<Option<usize>, codec::Error> { Ok(Some(self.data.len().saturating_sub(self.offset))) }
    ...
    fn read_byte(&mut self) -> Result<u8, codec::Error> {
		if self.offset + 1 > self.data.len() { return Err("out of data".into()) }
...
impl Bitmap {
	pub fn decode(data: &[u8]) -> Result<Self, codec::Error> {
		...
		if value == 0 { Err("Bitmap without a child.".into()) } else { Ok(Bitmap(value)) }
	}
...
// Implementasi lengkap dari trait NodeCodecT (sebelumnya NodeCodec)
...
where H: Hasher,
{
	...
	fn hashed_null_node() -> H::Out { H::hash(Self::empty_node()) }
    fn empty_node() -> &'static [u8] { &[trie_constants::EMPTY_TRIE] }
	fn is_empty_node(data: &[u8]) -> bool { data == Self::empty_node() }
    ...
    fn extension_node( _p: impl Iterator<Item=u8>, _nb: usize, _c: ChildReference<H::Out>) -> Vec<u8> { unreachable!("Codec without extension.") }
    fn branch_node( _c: impl Iterator<Item=impl Borrow<Option<ChildReference<H::Out>>>>, _v: Option<Value> ) -> Vec<u8> { unreachable!("Codec without extension.") }
    
    fn branch_node_nibbled( partial: impl Iterator<Item = u8>, number_nibble: usize, children: impl Iterator<Item = impl Borrow<Option<ChildReference<H::Out>>>>, value: Option<Value> ) -> Vec<u8> {
	...
	fn decode_plan(data: &[u8]) -> Result<NodePlan, Self::Error> {
		...
		let branch_has_value = if let NodeHeader::Branch(has_value, _) = &header { *has_value } else { true };
		match header {
			NodeHeader::Null => Ok(NodePlan::Empty),
			NodeHeader::HashedValueBranch(nibble_count) | NodeHeader::Branch(_, nibble_count) => {
				let padding = nibble_count % nibble_ops::NIBBLE_PER_BYTE != 0;
				if padding && nibble_ops::pad_left(data[input.offset]) != 0 { return Err(codec::Error::from("Bad format")) }
				let partial = input.take( (nibble_count + (nibble_ops::NIBBLE_PER_BYTE - 1)) / nibble_ops::NIBBLE_PER_BYTE, )?;
				...
				let mut children = [ None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, ];
				for i in 0..nibble_ops::NIBBLE_LENGTH {
					if bitmap.value_at(i) {
						let count = <Compact<u32>>::decode(&mut input)?.0 as usize;
						let range = input.take(count)?;
						children[i] = Some(if count == H::LENGTH { NodeHandlePlan::Hash(range) } else { NodeHandlePlan::Inline(range) });
					}
				}
				Ok(NodePlan::NibbledBranch { partial: NibbleSlicePlan::new(partial, partial_padding), value, children, })
			},
			NodeHeader::HashedValueLeaf(nibble_count) | NodeHeader::Leaf(nibble_count) => {
				...
				Ok(NodePlan::Leaf { partial: NibbleSlicePlan::new(partial, partial_padding), value, })
			},
		}
    }
}
...
// Helper functions
fn partial_from_iterator_encode<I: Iterator<Item = u8>>( partial: I, nibble_count: usize, node_kind: NodeKind, ) -> Vec<u8> {
...
impl Encode for NodeHeader {
	fn encode_to<T: Output + ?Sized>(&self, output: &mut T) {
		match self {
			...
			NodeHeader::Branch(true, nibble_count) => encode_size_and_prefix(*nibble_count, trie_constants::BRANCH_WITH_MASK, 2, output),
			NodeHeader::Branch(false, nibble_count) => encode_size_and_prefix(*nibble_count, trie_constants::BRANCH_WITHOUT_MASK, 2, output),
			NodeHeader::Leaf(nibble_count) => encode_size_and_prefix(*nibble_count, trie_constants::LEAF_PREFIX_MASK, 2, output),
			NodeHeader::HashedValueBranch(nibble_count) => encode_size_and_prefix(*nibble_count, trie_constants::ALT_HASHING_BRANCH_WITH_MASK, 4, output),
			NodeHeader::HashedValueLeaf(nibble_count) => encode_size_and_prefix(*nibble_count, trie_constants::ALT_HASHING_LEAF_PREFIX_MASK, 3, output),
		}
	}
}
...
impl Decode for NodeHeader {
	fn decode<I: Input>(input: &mut I) -> Result<Self, codec::Error> {
		...
		if i == trie_constants::EMPTY_TRIE { return Ok(NodeHeader::Null) }
		match i & (0b11 << 6) {
			...
			trie_constants::EMPTY_TRIE => {
				if i & (0b111 << 5) == trie_constants::ALT_HASHING_LEAF_PREFIX_MASK { Ok(NodeHeader::HashedValueLeaf(decode_size(i, input, 3)?)) }
                else if i & (0b1111 << 4) == trie_constants::ALT_HASHING_BRANCH_WITH_MASK { Ok(NodeHeader::HashedValueBranch(decode_size(i, input, 4)?)) }
                else { Err("Unallowed encoding".into()) }
			},
			_ => unreachable!(),
		}
	}
...
pub(crate) fn size_and_prefix_iterator( size: usize, prefix: u8, prefix_mask: usize, ) -> impl Iterator<Item = u8> {
	...
	let (first_byte, mut rem) = if size == l1 { (once(prefix + l1 as u8), 0) } else { (once(prefix + max_value as u8), size - l1) };
	let next_bytes = move || {
		if rem > 0 {
			if rem < 256 {
				let result = rem - 1;
...
fn encode_size_and_prefix<W>(size: usize, prefix: u8, prefix_mask: usize, out: &mut W) where W: Output + ?Sized, {
...
fn decode_size( first: u8, input: &mut impl Input, prefix_mask: usize, ) -> Result<usize, codec::Error> {
	...
	if result < max_value as usize { return Ok(result) }
	result -= 1;
	loop {
		let n = input.read_byte()? as usize;
		if n < 255 { return Ok(result + n + 1) }
		result += 255;
	}
}