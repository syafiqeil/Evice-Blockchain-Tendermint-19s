...
impl fmt::Debug for HostState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HostState")
         ...
         .field("memory", &self.memory.is_some()) // Hanya tampilkan apakah memori ada
         .field("return_data", &self.return_data)
         .field("logs", &self.logs)
         .finish_non_exhaustive() // Menandakan bahwa ada field yang tidak ditampilkan
    }
}
...
// --- FUNGSI HOST (API untuk Smart Contract) ---
...
fn log(env: FunctionEnvMut<Arc<Mutex<HostState>>>, message_ptr: u32, message_len: u32) {
    ...
    // PERBAIKAN: Gunakan `read` manual, bukan `read_string`
...
fn revert(env: FunctionEnvMut<Arc<Mutex<HostState>>>, message_ptr: u32, message_len: u32) {
    ...
    // Gunakan panic khusus untuk menandakan revert
...
pub fn execute_contract(
    ...
    match execution_result {
        ...
        Ok(Err(e)) => { // Error runtime Wasmer (misal: out of gas, trap)
            Ok(ExecutionResult {
                ...
                logs: Vec::new(),
                reverted: true,
                revert_message: e.to_string(), // Laporkan error Wasmer
                requested_transfers: Vec::new(),
            })
        }
        Err(panic_info) => { // Kontrak revert/panic
