...
use trie_db::{
    ...
    Trie, TrieMut, TrieDBBuilder, TrieDBMutBuilder,
    TrieDBNodeIterator, TrieError, DBValue
};
...
use crate::{
    Address,
    genesis::Genesis,
    crypto::{ADDRESS_SIZE, public_key_to_address, PUBLIC_KEY_SIZE},
...
// Fungsi helper untuk meng-hash kunci
...
pub struct ParityDbTrieBackend {
    db: Arc<Db>,
    // Buffering untuk operasi write yang belum di-commit
    pending_writes: std::collections::HashMap<TrieRoot, DBValue>,
    col: ColumnId,
}
...
impl ParityDbTrieBackend {
    ...
    // Method untuk commit pending writes ke database
    ...
    // Method untuk clear pending writes tanpa commit (rollback)
...
impl HashDBRef<KeccakHasher, DBValue> for ParityDbTrieBackend {
    fn get(&self, key: &TrieRoot, _prefix: Prefix) -> Option<DBValue> {
        // Cek pending writes dulu (uncommitted data)
        ...
        match self.db.get(self.col, key.as_ref()) {
            ...
            Ok(None) => None,
            Err(e) => {
                debug!("Database read error for key {}: {}", hex::encode(key.as_ref()), e);
                None
            }
        }
    }
...
impl HashDB<KeccakHasher, DBValue> for ParityDbTrieBackend {
    ...
    // --- Buffer writes instead of immediate commit ---
    fn emplace(&mut self, key: TrieRoot, _prefix: Prefix, value: DBValue) {
        // Simpan ke buffer pending writes, jangan langsung commit
    ...
    fn remove(&mut self, key: &TrieRoot, _prefix: Prefix) {
        // Untuk remove, insert None value ke buffer
...
// Konstanta Kolom untuk parity-db 
...
pub const ACTIVE_SEQUENCERS_KEY: &[u8] = b"active_sequencers_set"; // <-- Kunci untuk menyimpan set sequencer aktif
pub const PENDING_UPGRADE_KEY: &[u8] = b"pending_runtime_upgrade"; // <-- Kunci untuk menyimpan info upgrade 
...
pub struct Account {
    ...
    /// Hash dari kode WASM jika akun ini adalah sebuah kontrak.
    #[serde(with = "serde_helpers::option_vec_u8")]
    pub code_hash: Option<Vec<u8>>, 
    /// State root dari penyimpanan (storage) milik kontrak ini.
    #[serde(with = "serde_helpers::option_vec_u8")]
...
impl StateMachine {
    pub fn new<P: AsRef<Path>>(path: P) -> Result<Self, StateError> {
        let mut opts = DbOptions::with_columns(path.as_ref(), 8);
        
        opts.columns = vec![
            // ... (kolom lain jika perlu diindeks di masa depan)
            // Kita perlu meng-iterasi proposal untuk memprosesnya setelah voting selesai.
            parity_db::ColumnOptions { btree_index: true, ..Default::default() }, // COL_TRIE = 0
            parity_db::ColumnOptions { ..Default::default() }, // COL_METADATA = 1
            parity_db::ColumnOptions { btree_index: true, ..Default::default() }, // COL_GOVERNANCE = 2
            parity_db::ColumnOptions { ..Default::default() }, // COL_BLOCKS = 3
            parity_db::ColumnOptions { btree_index: true, ..Default::default() }, // COL_STATE_JOURNAL = 4
            parity_db::ColumnOptions { ..Default::default() }, // COL_L2_BATCHES = 5
            parity_db::ColumnOptions { ..Default::default() }, // COL_CONTRACT_CODE = 6
            parity_db::ColumnOptions { btree_index: true, ..Default::default() }, // COL_CONTRACT_STORAGE = 7
        ];
        ...
        if is_new_db {
            // Jika DB baru, muat dari genesis.json
            info!("Database kosong terdeteksi. Mencoba memuat dari genesis.json...");
            let genesis = Genesis::from_file("genesis.json")
                .map_err(|e| StateError::Io(std::io::Error::new(std::io::ErrorKind::NotFound, e.to_string())))?;
            machine.initialize_from_genesis(&genesis).map_err(|e| StateError::LogicError(e.to_string()))?;
        } else {
            // Jika DB sudah ada, muat state yang ada seperti biasa
            info!("Memuat state dari database yang ada...");
            
            // Muat state root (ini wajib ada jika DB tidak baru)
            let root_bytes = db.get(COL_METADATA, STATE_ROOT_KEY)?.expect("State root tidak ditemukan di database yang ada");
            machine.state_root.as_mut().copy_from_slice(&root_bytes);

            // Muat data opsional, gunakan default jika tidak ada
            machine.validators = db.get(COL_METADATA, VALIDATORS_KEY)?
                .map(|enc| bincode::deserialize(&enc).unwrap_or_default())
                .unwrap_or_default();

            machine.validator_last_seen = db.get(COL_METADATA, VALIDATOR_LAST_SEEN_KEY)?
                .map(|enc| bincode::deserialize(&enc).unwrap_or_default())
                .unwrap_or_default();

            machine.jailed_validators = db.get(COL_METADATA, JAILED_VALIDATORS_KEY)?
                .map(|enc| bincode::deserialize(&enc).unwrap_or_default())
                .unwrap_or_default();

            machine.active_sequencers = db.get(COL_METADATA, ACTIVE_SEQUENCERS_KEY)?
                .map(|enc| bincode::deserialize(&enc).unwrap_or_default())
                .unwrap_or_default();

            machine.l2_state_root = db.get(COL_METADATA, L2_STATE_ROOT_KEY)?.unwrap_or(vec![0; 32]);
            machine.l2_state_root_history = db.get(COL_METADATA, b"l2_state_root_history")?
                .map(|enc| bincode::deserialize(&enc).unwrap_or_default())
                .unwrap_or_else(|| {
                    // Jika riwayat tidak ada, inisialisasi dengan state root saat ini
    ...
    // --- Fungsi untuk menandai semua node yang dapat dijangkau ---
    fn mark_reachable_nodes(db: &Arc<Db>, root: &TrieRoot, reachable_nodes: &mut HashSet<Vec<u8>>) -> Result<(), StateError> {
        if !reachable_nodes.insert(root.as_ref().to_vec()) {
            return Ok(()); // Jika root ini sudah dikunjungi, hentikan.
        }
            ...
            match node.node() {
                Node::Extension(_, child_handle) => {
                    if let NodeHandle::Hash(hash) = child_handle {
                        let hash_arr: TrieRoot = (*hash).try_into().map_err(|_| StateError::TrieError("Invalid hash length".into()))?;
                        StateMachine::mark_reachable_nodes(db, &hash_arr, reachable_nodes)?;
                    }
                },
                Node::Leaf(_, Value::Node(child_handle_bytes)) => {
                    let hash_arr: TrieRoot = child_handle_bytes.try_into().map_err(|_| StateError::TrieError("Invalid hash length".into()))?;
                    StateMachine::mark_reachable_nodes(db, &hash_arr, reachable_nodes)?;
                },
                Node::Branch(children, value) | Node::NibbledBranch(_, children, value) => {
                    if let Some(Value::Node(val_hash)) = value {
                        let val_hash_arr: TrieRoot = val_hash.try_into().map_err(|_| StateError::TrieError("Invalid hash length".into()))?;
    ...
    // --- Fungsi utama untuk state pruning ---
    pub fn prune(&self, keep_recent_blocks: u64) -> Result<(), StateError> {
        ...
        // 1. MARK (TANDAI): Kumpulkan semua state root yang ingin disimpan.
        let mut reachable_nodes = HashSet::new();
        let first_kept_block = latest_block_num - keep_recent_blocks;

        info!("STATE PRUNING: Menjaga state dari blok #{} hingga #{}.", first_kept_block, latest_block_num);
        ...
        info!("STATE PRUNING: Total {} node trie ditandai sebagai dapat dijangkau.", reachable_nodes.len());

        // 2. SWEEP (SAPU): Hapus semua node yang tidak ditandai.
        ...
        if !delete_batch.is_empty() {
            info!("STATE PRUNING: Menghapus {} dari {} node trie yang usang...", delete_batch.len(), total_nodes);
            self.db.commit(delete_batch)?;
        } else {
            info!("STATE PRUNING: Tidak ada node usang yang ditemukan.");
        }

        // 3. Hapus entri jurnal state root yang lama
        ...
        if !journal_delete_batch.is_empty() {
            info!("STATE PRUNING: Menghapus {} entri jurnal state root yang lama.", journal_delete_batch.len());
    ...
    pub fn get_proposal(&self, id: ProposalId) -> Result<Option<ProposalState>, StateError> {
        match self.db.get(COL_GOVERNANCE, &id.to_be_bytes())? {
            Some(encoded) => Ok(Some(bincode::deserialize(&encoded)?)),
            None => Ok(None),
        }
    }

    pub fn set_proposal_in_tx(&self, ops: &mut Vec<(ColumnId, Vec<u8>, Option<Vec<u8>>)>, state: &ProposalState) -> Result<(), StateError> {
    ...
    pub fn get_all_proposals(&self) -> Result<Vec<ProposalState>, StateError> {
        ...
        while let Ok(Some((key, value))) = iter.next() {
            ...
            let proposal_state: ProposalState = bincode::deserialize(&value[..])?;
    ...
    pub fn process_finished_proposals(
        ...
        for mut proposal_state in proposals_to_check {
            if !proposal_state.executed && current_block_height > proposal_state.end_block {
                info!("Memproses proposal #{} yang telah selesai.", proposal_state.id);
    ...
    fn tally_and_execute_proposal(
        ...
        if proposal_state.yes_votes > proposal_state.no_votes {
            info!("Proposal #{} disetujui. Mengeksekusi tindakan...", proposal_state.id);
    ...
    pub fn initialize_from_genesis(&mut self, genesis: &Genesis) -> Result<(), Box<dyn std::error::Error>> {
        ...
        
        {
            ...
            for (pub_key_hex, genesis_account) in &genesis.accounts {
                ...
                let account = Account {
                    ...
                    nonce: 0,
                    signing_public_key: pub_key_bytes.clone(),
                    vrf_public_key: genesis_account.vrf_public_key
                        .as_ref()
        ...
        // Transfer dari MemoryDB ke ParityDB
        ...
        let genesis_block = crate::blockchain::Block {
            header: crate::blockchain::BlockHeader {
                ...
                // Kita bisa biarkan authority kosong atau menunjuk ke salah satu validator
        ...
        db_ops.push((COL_BLOCKS, 0u64.to_be_bytes().to_vec(), Some(bincode::serialize(&genesis_block)?)));

        // Simpan metadata penting lainnya
        db_ops.push((COL_METADATA, VALIDATORS_KEY.to_vec(), Some(bincode::serialize(&self.validators)?)));
        db_ops.push((COL_METADATA, STATE_ROOT_KEY.to_vec(), Some(self.state_root.as_ref().to_vec())));
        db_ops.push((COL_STATE_JOURNAL, 0u64.to_be_bytes().to_vec(), Some(self.state_root.as_ref().to_vec())));

        self.db.commit(db_ops)?;
        info!("Inisialisasi dari genesis selesai. State root: {}", hex::encode(self.state_root.as_ref()));
        Ok(())
    }
    ...
    pub fn check_for_pending_upgrade(&self, current_height: u64) -> Option<UpgradeInfo> {
        if let Ok(Some(data)) = self.db.get(COL_METADATA, PENDING_UPGRADE_KEY) {
            if let Ok(info) = bincode::deserialize::<UpgradeInfo>(&data) {
...
// --- TrieSession untuk operasi batch yang efisien ---
...
impl TrieSession {
    ...
    pub fn insert(&mut self, key: &[u8], value: &[u8]) -> Result<(), StateError> {
        let new_root = {
            let mut trie = TrieDBMutBuilder::<EviceTrieLayout>::from_existing(&mut self.backend, &mut self.root).build();
    ...
    pub fn get_account(&self, address: &Address) -> Result<Option<Account>, StateError> {
        ...
        match self.get(hashed_key.as_ref()).map_err(|e| StateError::TrieError(format!("{:?}", e)))? {
            Some(encoded_account) => {
                let account: Account = bincode::deserialize(&encoded_account)?;
                Ok(Some(account))
            }
    ...
    pub fn set_account(&mut self, address: &Address, account: &Account) -> Result<(), StateError> {
        let account_data = bincode::serialize(account)?;
        let hashed_key = KeccakHasher::hash(address.as_ref());
        self.insert(hashed_key.as_ref(), &account_data).map_err(|e| StateError::TrieError(format!("{:?}", e)))?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_apply_transaction_increments_nonce() {
        ...
        let tx_data = TransactionData::Transfer { recipient: TREASURY_ADDRESS, amount: 100 };
    ...
    fn test_bootstrap_genesis_state() {
        ...
        state.bootstrap_genesis_state().expect("Bootstrap should succeed");
...
// 1. ParityDbTrieBackend dengan pending writes buffer - mengatasi masalah reference-trie
// 2. TrieSession untuk batch operations - lebih efisien untuk multiple operations  
// 3. Hybrid approach: MemoryDB untuk bootstrap, ParityDbTrieBackend untuk production
// 4. Proper commit/rollback semantics - memungkinkan transaction-like operations